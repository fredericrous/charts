...
|-------|---------|-------------|-------------|
| [sync-bridge](charts/sync-bridge/) | 0.0.5 | 0.0.5 | Node service that bridges browser WebSocket clients to Hyperswarm for Yjs document sync in the website-builder. Single-pod for v1 (per PLAN.md "v1 scaling posture"). The Hyperswarm DHT layer requires UDP egress for hole-punching, so production deployments use `hostNetwork: true` or a NodePort with UDP forwarding — see values.yaml. |
| [builder-webapp](charts/builder-webapp/) | 0.0.19 | 0.0.19 | website-builder SPA + auth backend (openauth client + session cookie) |
